  ...
  if (plot.type == "line") {
    ...
    base_plot <- ggplot2::ggplot(plot_df,
                                 ggplot2::aes(x = exp, y = score,
                                              color = ident, group = ident)) +
      ggplot2::geom_line() +
      ggplot2::geom_point() +
      ggplot2::labs(color = "Predictor-target pair",
                    x = "", y = "Prediction Score")

    if (!is.null(facet) && facet == "predictor") {
      base_plot <- base_plot + ggplot2::facet_wrap(~predictor)
    }

    if (!is.null(facet) && facet == "target") {
      base_plot <- base_plot + ggplot2::facet_wrap(~target)
    }
  }
